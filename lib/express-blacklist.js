...
module.exports = {
	...
	blockRequests: function(blacklistFilename) {
		...
		var interceptor = function(request, response, next) {
			if (self.isInBlacklist(request.connection.remoteAddress)) {
				self.logEvent('warn', 'Rejecting request from ' + request.connection.remoteAddress + ', path and query was ' + request.originalUrl);
